...
struct vector_3 : xyz_float_t {
  ...
  vector_3(const xyz_float_t  &in) { set(in.x, in.y, TERN(HAS_Z_AXIS, in.z, 0)); }
  vector_3(const xyze_float_t &in) { set(in.x, in.y, TERN(HAS_Z_AXIS, in.z, 0)); }
  vector_3() { reset(); }
...
struct matrix_3x3 {
  ...
  void apply_rotation_xyz(float &x, float &y OPTARG(HAS_Z_AXIS, float &z));

  FORCE_INLINE void apply_rotation_xyz(xyz_pos_t &pos) { apply_rotation_xyz(pos.x, pos.y OPTARG(HAS_Z_AXIS, pos.z)); }
};
